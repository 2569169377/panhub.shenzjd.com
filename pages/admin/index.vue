  ...
  <div class="admin-page">
    <div class="admin-card">
      ...
      <div class="tabs">
        <button
          type="button"
          :class="['tab', { active: activeTab === 'search-log' }]"
          @click="activeTab = 'search-log'">
          搜索记录
        </button>
        <button
          type="button"
          :class="['tab', { active: activeTab === 'blacklist' }]"
      ...
      <div v-if="activeTab === 'blacklist'">
        ...
        <div class="result-head">
          <span>共 {{ blTotal }} 条记录（封禁中 + 惯犯档案 + 计数记录）</span>
...
.admin-card h1 { font-size: 22px; margin: 0 0 4px; }
...
.query-input { flex: 1; padding: 9px 12px; border: 1px solid var(--border-color, #e5e7eb); border-radius: 8px; font-size: 14px; background: var(--input-bg, #fff); color: var(--text-color, #1f2937); }
...
.query-btn { padding: 9px 18px; border: none; border-radius: 8px; background: var(--accent-bg, #2563eb); color: #fff; font-size: 14px; cursor: pointer; }
.query-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.result-head { display: flex; align-items: center; justify-content: space-between; margin: 14px 0 8px; font-size: 13px; color: var(--muted-color, #6b7280); }
...
.result-table th, .result-table td { text-align: left; padding: 8px 10px; border-bottom: 1px solid var(--border-color, #eee); white-space: nowrap; }
.result-table th { color: var(--muted-color, #6b7280); font-weight: 500; }
