...
export class MemoryCache<T = unknown> {
  ...
  /**
   * 递归估算值占用的近似内存（字节）。
   * 修正（2026-08-18）：旧实现嵌套数组按 length*64 估算，对 SearchResult[]
   * 这类对象数组严重低估（每条含 links/title 等字段，真实占用是估算的几十倍），
   * 导致 maxMemoryBytes 形同虚设、缓存实际吃掉数 GB 内存。
   * 新实现递归到字段级，估算值接近真实占用。搜索同一关键词重复率低，
   * 缓存收益有限，按真实大小收紧内存上限，避免缓存成为内存隐患。
   */
